/* Demos of fork(), exec(), waitpid(), kill() and backgound process
   ps a
   ./inf tag interval
   "inf.c" prints the tag every (interval) seconds. The default maximum is 10
   times.
...
int main() {
  ...
  while (cont == 'y') {
    ...
    if (pid == 0) {
      ...
      printf("Child finished.\n"); // This line won't work since exec()
                                   // successfully runs and replaces the code of
                                   // child process
      exit(EXIT_FAILURE);
    } else if (pid > 0) {
    ...
    setbuf(stdin, NULL); // clear the buffer. You could try to remove this line
                         // to see the result
    scanf("%c", &cont);
